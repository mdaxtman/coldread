...
For two or more runs, compute a trend indicator for the Pipeline column by comparing each run's pipeline composite to the previous run:
- `↑` if composite improved by more than 0.1
- `↓` if composite declined by more than 0.1
- `→` if within 0.1 of previous

For the first run, no trend indicator.
...
<job-slug>
─────────────────────────────────────────────────────────────────────
Run             Pipeline    Control     Delta    Winner
