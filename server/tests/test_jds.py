...
def test_get_latest_resume(mock_get_jd: Any, mock_get: Any) -> None:
    ...
    # screener_report becomes screenerReport in JSON (dict[str, Any] is not aliased)
    assert data["screenerReport"]["screener_analysis"]["semantic_score"] == 0.82
...
def test_list_resume_variants(mock_get_jd: Any, mock_list: Any) -> None:
    ...
    data = response.json()
    assert len(data) == 1
    assert data[0]["screenerReport"]["screener_analysis"]["overall_score"] == 0.78
